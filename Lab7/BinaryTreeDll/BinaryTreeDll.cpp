...
DWORD WINAPI ThreadFunc(PVOID p)
{
	...
	while (WaitForSingleObject(stopThread, 0)!=WAIT_OBJECT_0) {
		...
		HANDLE fileTo = CreateFile(L"C:\\Users\\Lera\\Desktop\\University\\Study\\Osi\\Lab7\\file.txt", GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
BinaryTree::BinaryTree(bool f)
{
	...
	buf_value = _strdup("");
	DWORD id;
	HANDLE fileTo = CreateFile(L"C:\\Users\\Lera\\Desktop\\University\\Study\\Osi\\Lab7\\file.txt", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	CloseHandle(fileTo);
